...
im_sprime_s_prime_with_mutations_table_sql_temp = _config['sql']['im_sprime_s_prime_with_mutations_table_sql_temp']
fnl_sprime_pooled_delta_sprime_table_sql_temp = _config['sql']['fnl_sprime_pooled_delta_sprime_table_sql_temp']
...
def refresh_omic_genes():
    ...
    try:
        ...
        logger.info(f"Total number of genes: {len(genes)}")
...
def refresh_secondary_dose_curve():
    ...
    try:
        logger.info(f"Data refresh process started for table '{table_name}'.")
        cursor.execute(drop_table_sql)
        pg_conn.commit()
        ...
        logger.info(f"Total number of records inserted into table '{table_name}': {total}")
        
    except Exception as e:
        logger.error(f"An error occurred while refreshing the '{table_name}' table.")
        traceback.print_exc()
        pg_conn.rollback()    
    finally:
        pg_conn.commit()
        ...
        logger.info(f"Completed refresh process for '{table_name}' in {(end_time_main - start_time_main).seconds} seconds.")
...
def refresh_s_prime():
    ...
    try:
        ...
        logger.info(f"Total number of rows inserted into '{table_name}': {total_rows}")
    except Exception as e:
        traceback.print_exc() 
        pg_conn.rollback()
    finally:
...
def refresh_damaging_mutations():
    ...
    try:
        ...
        refresh_s_prime_mutations_data_efficient("LUNG", df_final, "INITIAL")
    except Exception as e:
        traceback.print_exc() 
        pg_conn.rollback()
    finally:
...
def merge_in_chunks(tissue, cell_line_mutations_df, s_prime_solved_df, chunk_size=25000):
    ...
    for start in range(0, len(cell_line_mutations_df), chunk_size):
        logger.info(f"Processing chunk - Chunk Size: {chunk_size}")
        ...
        chunk_merged.drop(columns=["depmap_id", "ccle_name"], inplace=True)

        # Rename the ID column
        chunk_merged.rename(columns={"id": "s_prime_id"}, inplace=True)
        chunk_merged["tissue"] = tissue

        # # Step 5: Convert s_prime_id to integer
        chunk_merged["mutation_value"] = chunk_merged["mutation_value"].astype(float).astype(int)

        #logger.info(f"'S-Prime Cell Line Mutations' data frame columns: {chunk_merged.columns.tolist()}")
        logger.info(f"'S-Prime Cell Line Mutations' data row count: {chunk_merged.shape[0]}")
...
def refresh_s_prime_mutations_data_efficient(tissue, cell_line_mutations_df, load_type):
    ...
    mutations_table_create_sql = im_sprime_s_prime_with_mutations_table_sql_temp
    ...
    try:
        ...
        s_prime_solved_df = pd.DataFrame(s_prime_solved_data, columns=["id", "depmap_id", "ccle_name"])

        cell_names_for_tissue = s_prime_solved_df['depmap_id'].unique()

        target_mutations_df = cell_line_mutations_df[(cell_line_mutations_df['cell_line'].isin(cell_names_for_tissue))]

        refresh_s_prime_mutations_data_efficient_helper(tissue, target_mutations_df, s_prime_solved_df, 1, 18916, 100)
    except Exception as e:
        traceback.print_exc() 
    finally:
        end_time = datetime.now()
        logger.info(f"Completed process. — Duration: {(end_time - start_time).seconds} seconds")

def refresh_s_prime_mutations_data_efficient_helper(tissue, cell_line_mutations_df, s_prime_solved_df, gene_id_start, gene_id_max, gene_id_increment):
    logger.info(f"refresh_s_prime_mutations_data_efficient_helper started for tissue={tissue}, gene_id_start={gene_id_start}, gene_id_max={gene_id_max}, gene_id_increment={gene_id_increment}")
    start_id = gene_id_start
    
    while start_id <= gene_id_max:
        end_id = (start_id + gene_id_increment) - 1
        logger.info(f"Processing gene ids between {start_id} and {end_id}")
        subset_df = cell_line_mutations_df[(cell_line_mutations_df['gene_id'] >= start_id) & (cell_line_mutations_df['gene_id'] <= end_id)]
        logger.info(f"subset_df length = {subset_df.shape[0]}")
        merge_in_chunks(tissue, subset_df, s_prime_solved_df)
        start_id = start_id + gene_id_increment


def refresh_pooled_s_prime_helper(tissue, sprime_mutations_df):
    try:
        
        logger.info(f"sprime_mutations_df columns: {sprime_mutations_df.columns}")
        pooled_s_prime_df =  prepare_pooled_delta_s_results(sprime_mutations_df)
        ...
        # with pg_conn.cursor() as cursor:
        #     cursor.copy_expert(
        #         f"""COPY fnl_sprime_pooled_delta_sprime_temp (name, gene_id, ref_pooled_s_prime, ref_median_s_prime, 
